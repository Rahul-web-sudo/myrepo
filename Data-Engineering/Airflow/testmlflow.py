...
def interact_with_mlflow():
    ...
    AWS_ENDPOINT_URL = "http://local-s3-service.ezdata-system.svc.cluster.local:30000"  # Corrected with quotes
...
def search_mlflow_runs():
    experiment_name = "rahul"
    experiment = mlflow.get_experiment_by_name(experiment_name)

    if experiment:
        runs = mlflow.search_runs(filter_string="tags.mlflow.runName = 'rahult'")
        logging.info(runs)
    else:
